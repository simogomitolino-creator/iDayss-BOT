...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (interaction.isButton()) {
      ...
      // ── TIER SELECTION buttons → apre modal ──
      if (interaction.customId.startsWith('sell_tier_')) {
        const tier = interaction.customId.replace('sell_tier_', '');
      ...
      if (interaction.customId.startsWith('buy_')) {
        ...
        const ch = await guild.channels.create({
          ...
          permissionOverwrites: [
            ...
            ...(process.env.STAFF_ROLE_ID   ? [{ id: process.env.STAFF_ROLE_ID,   allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] }] : []),
            ...(process.env.OWNER_ROLE_ID   ? [{ id: process.env.OWNER_ROLE_ID,   allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] }] : []),
            ...(process.env.BOOSTER_ROLE_ID ? [{ id: process.env.BOOSTER_ROLE_ID, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] }] : []),
            ...(process.env.STAFFER_ROLE_ID ? [{ id: process.env.STAFFER_ROLE_ID, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages] }] : []),
          ],
        });
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith('sell_account_modal_')) {
        ...
        const statsLine = interaction.fields.getTextInputValue('acc_stats');
        const extra    = interaction.fields.getTextInputValue('acc_extra') || '';

        const [p11 = '?', hypercharge = '?', master = 'N/A'] = statsLine.split('|').map(s => s.trim());
        ...
        const imgChannel = await guild.channels.create({
          ...
          permissionOverwrites: [
            ...
            ...(process.env.STAFF_ROLE_ID   ? [{ id: process.env.STAFF_ROLE_ID,   allow: [PermissionFlagsBits.ViewChannel] }] : []),
            ...(process.env.OWNER_ROLE_ID   ? [{ id: process.env.OWNER_ROLE_ID,   allow: [PermissionFlagsBits.ViewChannel] }] : []),
            ...(process.env.BOOSTER_ROLE_ID ? [{ id: process.env.BOOSTER_ROLE_ID, allow: [PermissionFlagsBits.ViewChannel] }] : []),
            ...(process.env.STAFFER_ROLE_ID ? [{ id: process.env.STAFFER_ROLE_ID, allow: [PermissionFlagsBits.ViewChannel] }] : []),
          ],
        });
